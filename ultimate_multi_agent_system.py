...
class DecompilationAgent(Agent):
    ...
    def analyze_structure(self, decompiled_dir):
        ...
        structure = {
            ...
            "safe_files": [],
            "critical_files": [],
            "layout_files": [],  # 新增：布局文件
            "drawable_files": []  # 新增：drawable文件
        }
            ...
            for file in files:
                ...
                if file.endswith('.smali'):
                    ...
                    if 'layout' in file_path:
                        structure["layout_files"].append(file_path)
                    elif 'drawable' in file_path:
                        structure["drawable_files"].append(file_path)
                    else:
...
class LayoutModificationAgent(Agent):
    """布局修改智能体 - 专门修改布局文件"""
    def __init__(self):
        super().__init__("LayoutModificationAgent", "布局修改专家")
    
    def process_task(self, task):
        if task["type"] == "modify_layouts":
            return self.modify_layout_files(task["files"])
    
    def modify_layout_files(self, files):
        """修改布局文件"""
        print(f"📐 {self.name} 开始修改布局文件...")
        ...
        for file_path in files:
            try:
                ...
                # 修改布局参数，但保持结构完整
                content = self.modify_layout_attributes(content)
                ...
                print(f"修改布局文件失败 {file_path}: {e}")
        
        return {"status": "success", "modifications": modifications}
    
    def modify_layout_attributes(self, content):
        """修改布局属性"""
        import re
        
        # 修改margin和padding值
        patterns = [
            (r'android:layout_margin="(\d+)dp"', lambda m: f'android:layout_margin="{random.randint(1, 20)}dp"'),
            (r'android:layout_marginLeft="(\d+)dp"', lambda m: f'android:layout_marginLeft="{random.randint(1, 20)}dp"'),
            (r'android:layout_marginRight="(\d+)dp"', lambda m: f'android:layout_marginRight="{random.randint(1, 20)}dp"'),
            (r'android:layout_marginTop="(\d+)dp"', lambda m: f'android:layout_marginTop="{random.randint(1, 20)}dp"'),
            (r'android:layout_marginBottom="(\d+)dp"', lambda m: f'android:layout_marginBottom="{random.randint(1, 20)}dp"'),
            (r'android:padding="(\d+)dp"', lambda m: f'android:padding="{random.randint(1, 20)}dp"'),
        ]
...
class CriticalModificationAgent(Agent):
    ...
    def modify_strings(self, content):
        ...
        def replace_string(match):
            ...
            if len(original) > 0:
                # 保持相似长度，避免破坏UI
                modified = ''.join(random.choices(string.ascii_letters + string.digits, k=max(1, len(original))))
            ...
            try:
                ...
                if len(data) > 100:
                    ...
                    if end_pos > start_pos:
                        # 随机修改少量字节
                        for i in range(min(5, (end_pos - start_pos) // 100)):
...
class TestingAgent(Agent):
    ...
    def verify_modifications(self, modifications):
        ...
        verification_results = {
            ...
            "modification_types": {},
            "effect_score": 0,
            "real_modifications": 0  # 新增：实际修改的文件数
        }
        ...
        for file_path in modifications:
            try:
                if os.path.exists(file_path):
                    ...
                    # 检查是否真的有修改内容
                    if self.has_real_modification(file_path):
                        verification_results["real_modifications"] += 1
    ...
    def has_real_modification(self, file_path):
        """检查文件是否有实际修改"""
        try:
            if file_path.endswith('.modified'):
                # 检查修改标记文件的内容
...
class AnalysisAgent(Agent):
    ...
    def analyze_results(self, test_results):
        ...
        if modifications:
            ...
            analysis["real_modifications"] = verification.get("real_modifications", 0)
            analysis["recommendations"].append(f"成功修改了{len(modifications)}个文件")
            
            if verification.get("real_modifications", 0) > 0:
                analysis["recommendations"].append(f"其中{verification.get('real_modifications', 0)}个文件有实际修改")
    ...
    def make_decision(self, context):
        ...
        real_modifications = analysis.get("real_modifications", 0)
        
        if success_rate > 0.8 and effect_score > 0.5 and crash_risk < 0.5 and real_modifications > 0:
            decision = "success"
            action = "完成测试，生成报告"
        elif crash_risk > 0.7:
            decision = "retry"
            action = "崩溃风险过高，减少二进制文件修改"
        elif real_modifications == 0:
            decision = "retry"
            action = "没有实际修改，增加修改强度"
    ...
    def generate_report(self, data):
        ...
        report = {
            ...
            "summary": {
                ...
                "real_modifications": data.get("real_modifications", 0),
        ...
        # 保存JSON报告
        report_file = f"ultimate_test_report_{int(time.time())}.json"
...
- 实际修改文件数: {report['summary']['real_modifications']}
...
"""
        ...
        human_report_file = f"ultimate_human_readable_report_{int(time.time())}.md"
    ...
    def package_results(self, files):
        ...
        zip_filename = f"ultimate_multi_agent_results_{int(time.time())}.zip"
...
class UltimateMultiAgentSystem:
    ...
    def __init__(self):
        self.agents = {
            ...
            "layout_modification": LayoutModificationAgent(),
        ...
        self.modified_apk = "ultimate_modified_game.apk"
        self.decompiled_dir = "ultimate_decompiled_apk"
        self.test_results_dir = "ultimate_test_results"
        ...
        try:
            ...
            for iteration in range(1, self.max_iterations + 1):
                ...
                if structure["safe_files"]:
                    safe_files = structure["safe_files"][:20]  # 增加数量
                ...
                # 布局文件修改
                if structure["layout_files"]:
                    layout_files = structure["layout_files"][:10]
                    layout_result = self.agents["layout_modification"].process_task({
                        "type": "modify_layouts",
                        "files": layout_files
                    })
                    if layout_result["status"] == "success":
                        modifications.extend(layout_result["modifications"])
                
                # 配置文件修改
                if structure["config_files"]:
                    config_files = structure["config_files"][:25]  # 增加数量
                ...
                if structure["critical_files"]:
                    critical_files = structure["critical_files"][:5]
                ...
                # 二进制文件修改（更谨慎）
                if structure["binary_files"] and iteration > 5:  # 第6次迭代才开始修改二进制文件
                    binary_files = structure["binary_files"][:1]  # 只取1个
                ...
                # 更新实际修改数量
                verification = verification_result.get("verification", {})
                self.test_data["real_modifications"] += verification.get("real_modifications", 0)
                
                print(f"📊 第{iteration}次迭代结果:")
                print(f"   修改文件: {len(modifications)}个")
                print(f"   实际修改: {verification.get('real_modifications', 0)}个")
            ...
            print(f"实际修改文件数: {self.test_data['real_modifications']}")
            ...
            print(f"❌ 终极版多智能体协作异常: {e}")
            return False

def main():
    """主函数"""
    system = UltimateMultiAgentSystem()
    success = system.run_collaborative_workflow()
    
    if success:
        print("🎉 终极版多智能体协作系统执行成功！")
        sys.exit(0)
    else:
        print("💥 终极版多智能体协作系统执行失败！")
        sys.exit(1)
